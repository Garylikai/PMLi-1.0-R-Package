#' @title Kim et al.'s Modified t-Statistic
#' @description \code{modified.t} is used to calculate Kim et al.'s modified
#'   t-statistic for partially matched samples, specified by giving a data frame
#'   or matrix, testing hypothesis, true mean and confidence level.
...
#'
#'   Kim B, Kim I, Lee S, Kim S, Rha S, Chung H. Statistical methods of
#'   translating microarray data into clinically relevant diagnostic information
#'   in colorectal cancer. \emph{Bioinformatics}. 2004; 21(4): 517-528. [PubMed:
#'   15374865]
...
#'
#'   \code{\link{weighted.z}}, \code{\link{corrected.z}},
...
#'
#' # Kim et al.'s modified t-statistic formula interface
#' modified.t(pm, "less", conf.level = 0.99)
#'
#' # p-value of Kim et al.'s modified t-statistic
#' p.value1 <- modified.t(pm)$p.value
#' @export

modified.t <- function(data, alternative = c("two.sided", "less", "greater"),
  ...
  to begin.\n")
    ...
    if(shapiro.test(unlist(data2[1, ]))$p.value <= 0.05){
      ...
      warning("Kim et al.'s modified t-statistic assumes the data contains
  ...
  least one of the samples, the p-value for the given data will
  be outputted using Wilcoxon signed rank test method for matched
  pairs.\n")
    }else{
  ...
  partially matched samples. The inputted data contains
  complete matched pairs but only the first sample is given.
  Because the Shapiro-Wilk test for the matched paired
  samples is significant, the p-value for the complete
  ...
  complete matched pairs but only the first sample is given.
  Because the Shapiro-Wilk test for the matched paired
  samples is not significant, the p-value for the complete
  matched pairs using the matched t-test method for matched
  pairs will be outputted. Because the Shapiro-Wilk test for
  the two independent samples is not significant and the F
  test to compare the variances of the independent samples is
  ...
  the two independent samples is not significant and the F
  test to compare the variances of the independent samples is
  not significant, the two independent samples using the
  two-sample t-test method with equal variance assumption
  will also be outputted.\n")
        }
      }
  ...
  will also be outputted.\n")
    ...
    d_bar <- mean(unlist(data1[1,])-unlist(data1[2,]))
    t_bar <- mean(unlist(data2[1,]))
    n_bar <- mean(unlist(data3[2,]))
    s_d <- sd(unlist(data1[1,])-unlist(data1[2,]))
    s_t <- sd(unlist(data2[1,]))
    s_n <- sd(unlist(data3[2,]))
    n_h <- 2/(1/n2+1/n3)
    
    t3 <- (n1*d_bar+n_h*(t_bar-n_bar)-mu)/
      sqrt(n1*s_d^2+n_h^2*(s_t^2/n2+s_n^2/n3))
    if(alternative == "two.sided"){
      ...
      p_value <- pnorm(t3)
      cint <- c(-Inf, t3+qnorm(conf.level))
    }else{
      p_value <- 1-pnorm(t3)
      cint <- c(t3-qnorm(conf.level), Inf)
    }
    ...
    PARAMETER <- NULL
    PVAL <- p_value
    CINT <- mu+cint*sqrt(n1*s_d^2+n_h^2*(s_t^2/n2+s_n^2/n3))
    attr(CINT, "conf.level") <- conf.level
    ESTIMATE <- mu+t3*sqrt(n1*s_d^2+n_h^2*(s_t^2/n2+s_n^2/n3))
    names(ESTIMATE) <- names(mu) <- "difference in means"
    STDERR <- sqrt(n1*s_d^2+n_h^2*(s_t^2/n2+s_n^2/n3))
    METHOD = "Kim et al.'s modified t-statistic"
  }
